...
class CritterI18n extends PolymerElement {
    static get template() {
        ...
        <div id="flag_container">
             ...
             <img src="/translation/flags/eg.jpeg" on-click="_selectLanguage" lang="ar-SA" style="width: 35px; height: 25px;"> <!-- Egyptian Flag -->
             </div>
        `;
    }
    ...
    static get properties() {
        return {
            supportedLanguages: {
                type: Array,
                value: ["de-DE", "en-US", "ar-SA"]  // Arabic added
            }
        }
    }
    ...
    connectedCallback() {
        ...
        if (!localStorage.getItem("language")) {
            if (this.supportedLanguages.includes(navigator.language)) {
                ...
                this._setLanguage("en-US");  // Default to English
            }
        } else {
    ...
    _setLanguage(lang) {
        // Handle the custom file naming
        if (lang === "ar-SA") {
            lang = "de-Ar";  // Map to the actual file name for Arabic
        }
    ...
    loadTranslationData(lang) {
        ...
        req.url = "/translation/" + lang + ".json";  // Use the correct file name
        req.method = "GET";
        req.handleAs = 'json';
