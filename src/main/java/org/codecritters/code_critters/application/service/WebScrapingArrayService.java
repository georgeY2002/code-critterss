...
@Service
public class WebScrapingArrayService {

    // Add your DeepL API key here
    private static final String DEEPL_API_KEY = "44c6646f-25ac-4abc-ab50-58b28d62aaef:fx";

    // Method to scrape content from the W3Schools Data Arrays page
    public String scrapeDataArraysPage() throws IOException {
        String url = "https://www.w3schools.com/dsa/dsa_data_arrays.php"; // Correct URL for arrays
     ...
     // Updated remove unwanted elements section
content.select("svg, .-user-session-btns, .user-progress-star, " +
        ...
        "#w3-exerciseform, .w3-btn.w3-margin-bottom, #vueApp, #vueApp2, #vueApp3, #vueApp4, " + // Added vueApp, vueApp2, vueApp3, vueApp4
    ...
    public String getContentInLanguage(String lang) throws IOException {
        String htmlContent = scrapeDataArraysPage();
...
public byte[] getScrapedContentAsPdf(String lang) throws IOException {
    ...
    // Set text direction based on your requirement
    String direction = "ltr"; // Left-To-Right

    // Wrap content into an HTML format with font settings
    String html = "<html><head><title>Data Arrays</title>" +
        "<style>" +
        "body { font-family: 'Tahoma', 'Arial', 'sans-serif'; direction: " + direction + "; }" +
        "</style>" +
    ...
    try (ByteArrayOutputStream baos = new ByteArrayOutputStream()) {
        ...
        fontProvider.addFont(FontProgramFactory.createFont(StandardFonts.HELVETICA));
